...
class Process():
  ...
  def parse_system_info(self, file):
    ...
    for line in file:
        if 'TSC Freq' in line or 'Processor Base Freq' in line or 'CPU Freq (detected)' in line:
            m = re.search('\D(\d+\.\d*)', line)
            system_info['system.tsc_freq'] = float(m.group(1)) * 1e6
            system_info['core freq'] = float(m.group(1)) / 1000.0
        elif 'cpu_family' in line:
        ...
        #elif 'CPU Freq (detected)' or 'Processor Base Freq' in line:
            #m = re.search('\D(\d+\.\d*)', line)
            #system_info['core freq'] = float(m.group(1)) / 1000.0
